...
export const FULL_PROJECTS: Project[] = [
  {
    ...
    problem: 'User research revealed that 70% of academic researchers struggle with "Context Switching Fatigue"—the energy lost moving between disconnected notes. Standard note-taking is fragmented; you lose the deeper connections that actually lead to understanding. Most people struggle to see how "Data Structures" relates to "Graph Theory" in a meaningful way. I saw a need for a tool that doesn’t just store facts, but actively maps the structural prerequisites of a subject. The problem wasn’t the availability of information, but the absence of a system that could visualize the "forgetting curve" and the interconnectivity of complex academic subjects.',
    process: 'The design process began with extensive user interviews and affinity mapping to identify the primary failure modes of existing tools. I spent weeks refining the "Mastery Index" sidebar, which categorizes topics from "Mastered" to "Weak" based on review frequency. I engineered a "Spaced Repetition" engine that generates cards like "NEXT BEST TOPIC: GRAPH THEORY," showing precise metrics such as "25d since review" and "~10 min review estimate." I also designed a "Create Concept" modal with an "Initial Clarity" slider (defaulting to 10%) and placeholder text to guide users. The interaction model was stress-tested to maintain a 0.4ms latency even with hundreds of active nodes.',
    outcome: 'The result is a fluid, intelligence-first platform that improved task-completion speed by 22% in early usability testing. The "Computer Science" graph successfully visualizes topics from Statistics to Machine Learning with weighted physical links. Recruiters can see the technical depth in the HUD, which tracks global progress across active subjects. The interaction is snappy and weighted, making the graph feel like a physical extension of the user’s mind. The final build includes a robust subject selector and a "Mastery Index" that provides a clear roadmap for long-term retention, transforming research from a chore into a high-performance cognitive activity.',
  ...
  },
  {
    ...
    problem: 'Eye-tracking studies showed that presenters lose audience trust every time they look away from the lens to read notes. Standard teleprompters are intrusive; they require dragging windows near the camera, which is imprecise and clunky. Every time a presenter looks down to read, they break the "fourth wall," losing audience trust. Existing software solutions lack the system-level integration needed to feel truly native on macOS. There was no tool that specifically addressed the geometry of the MacBook Pro notch, leading to janky workarounds like sticky notes or shrunken documents.',
  ...
  },
  {
    ...
    problem: 'User research highlighted a growing trend of "Discovery Fatigue"—users feel overwhelmed by infinite recommendations. Streaming apps are engineered for infinite discovery at the expense of focused listening. The flat, gesture-based touchscreen model also lacks the tactile satisfaction of real physical hardware. There was a clear gap for a dedicated web player with genuine personality — one that respects the listener\'s own choices rather than second-guessing them.',
    process: 'The process involved a deep dive into hardware UI patterns, analog signal design, and high-roughness material aesthetics. I engineered a custom boot sequence in React 19 that simulates a hardware power-up with diagnostic signal feedback. The dial physics were the most time-intensive component — tuning Framer Motion easing curves until the rotational inertia felt genuinely heavy rather than just animated. I mapped out the interaction architecture to ensure every "click" provided haptic-style visual feedback.',
    outcome: 'The result is a tactile, analog-inspired web player that creates a dedicated zone for intentional listening. It successfully removes the cognitive load of infinite choices, resulting in 3x higher session engagement for album-length listening in beta testing. No algorithm. No noise. Just control.',
  ...
  },
  {
    ...
    problem: 'Research into hiring workflows revealed that 60% of resumes fail the first "ATS screen" due to multi-column layout garbling. Furthermore, the resume market is a binary of bad choices: either rigid templates that break when you add a bullet point, or loose word processors that fail ATS parsing. Job seekers are forced to hand over their data to companies that might sell it, just to get a basic document formatted.',
